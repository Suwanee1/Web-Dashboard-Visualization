...
data = [go.Scatter(x=new_df['Recovered'], y=new_df['Unrecovered'], text=new_df['Country'], mode='markers',
                   marker=dict(size=new_df['Confirmed'] / 100, color=new_df['Confirmed'] / 100, showscale=True))]
layout = go.Layout(title='Corona Virus Confirmed Cases', xaxis_title="Recovered Cases", yaxis_title="Unrecovered Cases",
                   hovermode='closest')
